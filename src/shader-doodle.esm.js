var Template = {
  ...
  css(w, h) {
    return "<style>\n      :host {\n        position: relative;\n        display: inline-block;\n        width: ".concat(w || 250, "px;\n        height: ").concat(h || 250, "px;\n      }\n      :host > canvas {\n        position: absolute;\n        top: 0;\n        left: 0;\n        height: 100%;\n        width: 100%;\n        border-radius: inherit;\n      }\n    </style>");
  }
...
function Program(gl, vs, fs, vertices, shadertoy = false) {
...
function Texture(gl, textureUnit, optsParam = {}) {
...
function GeneralTexture(renderer, textureUnit, name, src, webcam, wrapS, wrapT, minFilter, magFilter, forceUpdate, shadowRoot) {
  const gl = renderer.gl;
  const texture = Texture(gl, textureUnit);
  let source;
  let type;
  ...
  function setupElementReference() {
    try {
      ...
      const root = selectedShadowRoot ? selectedShadowRoot.shadowRoot : document;
...
class TextureElement extends SDBaseElement {
  static get observedAttributes() {
    return ['mag-filter', 'min-filter', 'name', 'shadow-root', 'src', 'wrap-s', 'wrap-t'];
  }
  ...
  init(program) {
    ...
    this.program = program;
    this.texture = GeneralTexture(this.renderer, program.getTexUnit(), this.name, this.src, this.webcam, this.wrapS, this.wrapT, this.minFilter, this.magFilter, this.forceUpdate, this.shadowRoot);
    program.addTexture(this.texture);
  }
...
class SDUniformElement extends SDBaseElement {
  ...
  set x(newx) {
    if (newx != null) this.setAttribute('x', newx); else this.removeAttribute('x');
  }
  ...
  set y(newy) {
    if (newy != null) this.setAttribute('y', newy); else this.removeAttribute('y');
  }
  ...
  set z(newz) {
    if (newz != null) this.setAttribute('z', newz); else this.removeAttribute('z');
  }
  ...
  set w(neww) {
    if (neww != null) this.setAttribute('w', neww); else this.removeAttribute('w');
  }
  ...
  set type(newType) {
    if (newType != null) this.setAttribute('type', newType); else this.removeAttribute('type');
  }
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    switch (name) {
      ...
      case 'z':
      case 'w':
        if (newValue != null) this.renderer.setUniform(this.name, this.getValue());
        break;
    }
  }
...
function Surface(element) {
  const canvas = element.canvas instanceof HTMLCanvasElement ? element.canvas : document.createElementNS('http://www.w3.org/1999/xhtml', 'canvas');
...
class ShaderDoodleElement extends SDNodeElement {
  ...
  attributeChangedCallback(name) {
    ...
    if ((name === 'height' || name === 'width') && styles.length > 0) {
      const val = this[name];
      styles[0].cssRules[0].style[name] = Number.isInteger(val) ? "".concat(val, "px") : '250px';
    }
  }
